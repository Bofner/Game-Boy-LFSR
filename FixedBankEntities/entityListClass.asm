...
;We shouldn't get to this part of the code
	...
	;Set entity to deactivate itself upon update unless properly initialized
		...
		add hl, de							;HL = entity.entityUpdatePointer
		ld a, lobyte(@DeactivateUninitializedEntity)
		ldi (hl), a							;HL = entity.entityUpdatePointer + 1
		ld a, hibyte(@DeactivateUninitializedEntity)
		ld (hl), a
		ld de, entityStructure.eventID - (entityStructure.entityUpdatePointer + 1)
		add hl, de							;HL = entity.NEXTFREE.eventID
...
;==============================================================
;
;Parameters: We should be coming from UpdateAllEntities, so DE = entity.eventID
;Returns: None
;Affects: TBD
